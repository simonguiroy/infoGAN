...
parser.add_argument('--model_path', type=str, default='./models') # path to save trained models
parser.add_argument('--image_path', type=str, default='/Tmp/guiroysi/infoGAN/CelebA/128_crop')  # path of data
...
def main():
    ...
    discriminator = Discriminator(args.cc_dim, args.dc_dim)
    ...
    for epoch in range(args.num_epochs):
        for i, images in enumerate(data_loader):
            ...
            g_loss = g_loss_a + 1.0 * d_loss_cc + 1.0 * d_loss_dc
        ...
        path_g = os.path.join(args.model_path, 'generator-%d.pkl' % (epoch + 1))
        torch.save(generator.state_dict(), path_g)
        
        path_d = os.path.join(args.model_path, 'discriminator-%d.pkl' % (epoch + 1))
